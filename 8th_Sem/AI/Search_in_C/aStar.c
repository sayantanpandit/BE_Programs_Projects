...
enqueP(qnode** rear, qnode** front, qnode newnode)
{
	if((*rear)==NULL)
	{
	...
	{
		qnode* check=(qnode*)malloc(sizeof(qnode));
		check=(*front);
		int flag=0;
		while(check!=NULL)
		{
			if(check->val < newnode.val)
			{
				flag=1;
				break;
			}
...
puzzle(int initMat[3][3], int goalMat[3][3])
{
	...
	for(i=0;i<3;i++)
	{
		for(j=0;j<3;j++)
		{
			if((first.board[i][j]!=goalMat[i][j]) && goalMat[i][j]!=0)
				h++;
		}
	}
	first.val=h+first.distance;
	enqueP(&r1,&f1,first);
	while(1)
	{
		count=count+1;
		qnode tempNode=deque(&r1,&f1);
		int flag1=0;
		for(i=0;i<3;i++)
		{
			for(j=0;j<3;j++)
		...
		if(flag1==1)
		{
			...
			if(posR+1 < 3)
			{
				...
				for(i=0;i<3;i++)
				{
					for(j=0;j<3;j++)
					{
						if((tempNode1.board[i][j]!=goalMat[i][j]) && goalMat[i][j]!=0)
							h++;
					}
				}
				tempNode1.val=h+tempNode.distance;
				tempF=f2;
				int flag2=0;
				while(tempF!=NULL)
				{
